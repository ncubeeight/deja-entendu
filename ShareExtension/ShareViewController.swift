...
/// extension from a Share Sheet. Handles three distinct cases:
...
///   - A PDF (e.g. Share on a document from Files or Mail) has its text
///     extracted and added as a text sample. Only works for PDFs with a
///     real text layer — a scanned/image-only PDF has nothing for PDFKit
///     to extract and falls through to "Couldn't read that", same as any
///     other unreadable attachment.
final class ShareViewController: UIViewController {
    ...
    private enum ImportOutcome {
        ...
        var message: String {
            ...
            case .textSampleAdded: return "Added to Déjà Entendu ✓"
    ...
    private func importSharedItem() async -> ImportOutcome {
        ...
        // PDF next — checked before the generic audio/movie file loader
        // below since a PDF only conforms to .pdf, not those types, but
        // ordering it first keeps the "most specific type wins" pattern
        // consistent with the plain-text check above.
        for provider in attachments where provider.hasItemConformingToTypeIdentifier(UTType.pdf.identifier) {
            if let fileURL = await loadFileURL(from: provider, typeIdentifier: UTType.pdf.identifier, fallbackExtension: "pdf"),
               let text = extractText(fromPDFAt: fileURL),
               !text.isEmpty {
                return saveTextSample(text) ? .textSampleAdded : .failed
            }
        }
        ...
        for provider in attachments {
            for type in candidateTypes where provider.hasItemConformingToTypeIdentifier(type.identifier) {
                if let fileURL = await loadFileURL(from: provider, typeIdentifier: type.identifier, fallbackExtension: "m4a") {
    ...
    private func extractText(fromPDFAt url: URL) -> String? {
    ...
    private func loadFileURL(from provider: NSItemProvider, typeIdentifier: String, fallbackExtension: String) async -> URL? {
        await withCheckedContinuation { continuation in
            provider.loadItem(forTypeIdentifier: typeIdentifier, options: nil) { item, _ in
                if let url = item as? URL {
                    continuation.resume(returning: url)
                } else if let data = item as? Data {
                    // Only hit when the provider hands back raw Data instead
                    // of a file URL — the extension here just needs to match
                    // the actual type so downstream readers (PDFDocument,
                    // AVAudioFile) don't choke on a mismatched one.
    ...
    private func saveTextSample(_ text: String) -> Bool {
        guard let container = FileManager.default
            .containerURL(forSecurityApplicationGroupIdentifier: appGroupID) else { return false }
